...
#elif defined(CONFIG_SOC_CH32L103)
	/* CH32L103 flash latency settings (verified against vendor EVT) */
	if (WCH_RCC_SYSCLK <= 24000000) {
		latency = FLASH_ACTLR_LATENCY_0;
...
/* Timeout for oscillator startup (vendor uses 0x1000) */
#define WCH_RCC_OSC_TIMEOUT 0x1000U
...
static int clock_control_wch_rcc_init(const struct device *dev)
{
	...
	while ((RCC->CTLR & RCC_PLLRDY) != 0 && timeout--) {
	}
#endif
	...
	timeout = WCH_RCC_OSC_TIMEOUT;
	while ((RCC->RSTSCKR & RCC_LSIRDY) == 0 && timeout--) {
	}

	if (IS_ENABLED(CONFIG_DT_HAS_WCH_CH32V00X_HSI_CLOCK_ENABLED)) {
		RCC->CTLR |= RCC_HSION;
		timeout = WCH_RCC_OSC_TIMEOUT;
		while ((RCC->CTLR & RCC_HSIRDY) == 0 && timeout--) {
		}
	}
	...
	if (IS_ENABLED(CONFIG_DT_HAS_WCH_CH32V00X_HSE_CLOCK_ENABLED)) {
		RCC->CTLR |= RCC_HSEON;
		timeout = WCH_RCC_OSC_TIMEOUT;
		while ((RCC->CTLR & RCC_HSERDY) == 0 && timeout--) {
		}
		if ((RCC->CTLR & RCC_HSERDY) == 0) {
			LOG_ERR("HSE failed to start");
			return -EIO;
		}
	}
	...
	if ((RCC->CTLR & RCC_PLLRDY) == 0) {
		LOG_ERR("PLL failed to lock");
		return -EIO;
	}
#endif
